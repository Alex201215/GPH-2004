...
class ImageCamera:
    ...
    def filtre_passe_bas(self, rayon_du_filtre) -> None:
        if len(self.image_fft) <= 1:
            ...
            # masque et ensuite on veut ke convoluer sur l'image.

            # Application du masque sur l'image
            fft_mask_multiply = np.multiply(self.image_fft, mask) / 255 # Masque brute
            ...
            back_ishift = np.fft.ifftshift(self.image_fft)
            ifft_mask = np.fft.ifftshift(fft_mask_multiply) # Pas encore sur de comprendre ifftshift
